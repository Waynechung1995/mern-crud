...
const Register = () => {
    ...
    let [passpord, setPasspord] = useState("");
    ...
    const handleChangePassword = (e) => {
        setPasspord(e.target.value);
    };
    const handleChangeRole = (e) => {
        setRole(e.target.value);
    };
    const handleRegister = () => {
        AuthService.register(username, email, passpord, role).then(() => {
            window.alert("Registration succeed. You are now redirected to the login page.");
            navigate("/login");
        }).catch((error) => {
